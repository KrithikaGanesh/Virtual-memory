...
petmem_deinit_process(struct mem_map * map)
{
	...
	struct vaddr_reg *del;
	list_for_each_safe(node, temp, &(map->track_memalloc)){
		del = list_entry(node, struct vaddr_reg, vm_list);
        free_pa(del->va_start);
        invlpg(del->va_start);
		list_del(node);
		kfree(del);
	}
...
void free_pa(uintptr_t address){
    ...
    pdpbase = (pdpe64_t *)( BASE_TO_PAGE_ADDR( pml->pdp_base_addr )) ;
    if(!pdp->present) {
        return;
    }
    pde = (pde64_t *)__va(BASE_TO_PAGE_ADDR( pdp->pd_base_addr ) + PDE64_INDEX( address )* 8);
    pdebase = (pde64_t *)(BASE_TO_PAGE_ADDR( pdp->pd_base_addr ));
    if(!pde->present) {
        return;
    }
    pte = (pte64_t *)__va( BASE_TO_PAGE_ADDR( pde->pt_base_addr ) + PTE64_INDEX( address ) * 8 );
    ptebase = (pte64_t *)( BASE_TO_PAGE_ADDR( pde->pt_base_addr ));
    if(!pte->present) {
        return;
    }
    data = (void *)( BASE_TO_PAGE_ADDR( pte->page_base_addr ) + PAGE_OFFSET_4KB( address ) );
    petmem_free_pages((uintptr_t)data, 1);
	...
	for(i = 0; i < 512; i++){
        int offset = i * 8;
        if(((pte64_t *)(__va(ptebase+offset)))->present == 1){
            present_flag = 1;
            break;
        }
    }
    if(!present_flag){        
       petmem_free_pages((uintptr_t)ptebase, 1);
    }
    else{
        return;
    }
	...
	for(i = 0; i < 512; i++){
        int offset = i * 8;
        if(((pde64_t *)(__va(pdebase+offset)))->present == 1){
            present_flag = 1;
            break;
		}
    }
     if(!present_flag){        
       petmem_free_pages((uintptr_t)pdebase, 1);
    }
    else{
    ...
    for(i = 0; i < 512; i++){
        int offset = i * 8;
        if(((pdpe64_t *)(__va(pdpbase+offset)))->present == 1){
            present_flag = 1;
            break;
		}
    }
    if(!present_flag){
       petmem_free_pages((uintptr_t)pdpbase, 1);
    }
    else{
        return;
    }
